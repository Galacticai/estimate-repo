...
function main() {
  ...
  authorsWithEmails.forEach(({ name, email }, index) => {
      console.log(`[${index + 1}/${authorsWithEmails.length}] Processing: ${name}`);
      ...
      const result = writer.generateDeveloperJson(name, commitsWithEstimations, filenameBase);
